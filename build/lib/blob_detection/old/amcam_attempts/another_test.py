#!/usr/bin/env python3
import nivision.core as nivision
import sys, ctypes
import numpy as np
...
def main(name):
    ...
    nivision.IMAQdxConfigureAcquisition(id, nivision.bool32(True), ctypes.c_uint32(1))
    nivision.IMAQdxStartAcquisition(id)
    nivision.IMAQdxGrab(id, img, nivision.bool32(True))
    data, _, _ = nivision.imaqImageToArray(img)
    nivision.IMAQdxStopAcquisition(id)
    nivision.IMAQdxCloseCamera(id)
    im = Image.frombuffer('L', (1920, 1520), data, 'raw', 'L', 0, 1)

    return np.array(im)


if __name__ == "__main__":
    name = "cam5".encode('utf-8')
    main(name)
